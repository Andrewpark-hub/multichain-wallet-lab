...
> - **문제 상황**: `Approval` 이벤트를 10만 블록 단위로 한 번에 조회하자 `-32701 exceed maximum block range: 50000` 오류가 발생하며 요청 자체가 거부되어 승인 감사가 불가능한 상태.
> - **원인 분석**: 공개 RPC 제공자가 `eth_getLogs` 호출당 조회 가능한 블록 범위를 제한하고 있음을 확인. 다만 그 한도가 제공자마다 달라(publicnode 50,000 / 다수의 상용 RPC 10,000) 특정 값에 맞추면 RPC 교체 시 다시 깨지는 구조임을 파악.
> - **해결 방안**: 어느 RPC에서도 통과하는 `LOG_CHUNK`(기본 9,000 블록) 단위로 구간을 분할해 순회하고, 실패한 구간은 경고만 출력한 뒤 계속 진행하도록 처리. 이 과정을 통해 **지갑 앱들이 왜 별도의 Indexer를 두는지** 를 직접 체감.

> **잔고 0 상태에서 `estimateGas` 선행 실패로 인한 실습 중단 문제**
> - **문제 상황**: faucet으로 테스트넷 ETH를 받기 전에는 `estimateGas` 가 `missing revert data` 오류로 실패해, 트랜잭션 구조 자체를 확인할 수 없는 문제 발생.
